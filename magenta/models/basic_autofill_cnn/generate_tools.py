...
tf.app.flags.DEFINE_string(
    'prime_fpath', 'condition_on/magenta_theme.xml',
    'Path to the Midi or MusicXML file that is used as a prime.')
tf.app.flags.DEFINE_string(
    'output_dir', '/usr/local/ /home/annahuang/magenta_tmp/generated/',
...
def regenerate_voice_by_voice(pianorolls, wrapped_model, config):
  """Rewrite a piece voice by voice.
...
def generate_routine(config, output_path):
  ...
  # Checking if there's inconsistency in the type of priming requested.
  ...
  # Get data.
  seeder = seed_tools.get_seeder(model_name)
  ...
  if start_with_empty:
    ...
    pianorolls, piece_names = seeder.get_random_batch(return_names=True)
    piece_name = os.path.split(piece_names[config.requested_index])[0]

  # Due to initial naming of piece names with N/A and the forward slash interpreted as a directory.
  print 'Name:', piece_name
  piece_name = piece_name.replace('N/A', 'N_A')
  ...
  for i in range(config.num_samples):
    ...
    generated_pianoroll, autofill_steps, original_pianoroll = generated_results

    # Synth original.
    if original_pianoroll is not None:
      original_seq = seeder.encoder.decode(original_pianoroll)
      fpath = os.path.join(output_path,
                           'original-%s-run_id_%s.midi' % (generate_method_name,
                                                           run_local_id))
    ...
    fpath = os.path.join(output_path,
                         'generated-%s-run_id_%s.midi' % (generate_method_name,
                                                          run_local_id))
