...
import tensorflow as tf
 .learning.brain.research.magenta.models.basic_autofill_cnn.summary_tools as summary_tools
...
class BasicAutofillCNNGenerateTest(tf.test.TestCase):
  ...
  def testAggregateMean(self):
    ...
    mean_aggregate = summary_tools.AggregateMean('test')
    for value in values:
  ...
  def checkAggregateInOutMaskPredictionPerformanceStats(self, targets,
    ...
    perf = summary_tools.AggregateInOutMaskPredictionPerformanceStats(
    ...
    for kind_str in ['', 'mask-']:
      self.assertEqual(
          perf.aggregates['%snoteons' % kind_str].predicted_positive_counts +
          (perf.aggregates['%snoteoffs' %
                           (kind_str)].predicted_positive_counts),
          perf.aggregates['%snoteons' % kind_str].instance_counts)
  ...
  def testAggregateInOutMaskPredictionPerformanceStats(self):
    ...
    prediction_cases = [self.all_zeros(), self.all_ones(),
                        self.all_ones() * 0.5, self.all_ones() * 0.49]
