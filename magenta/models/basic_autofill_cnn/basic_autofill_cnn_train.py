r"""Trains onvolution net model class for music autofill.
...
tf.app.flags.DEFINE_string(
 'input_dir',
 '/tmpBachChorales/instrs=4_duration=0.250_sep=True/',
...
tf.app.flags.DEFINE_integer('use_residual', 0,
       '1 specifies use residual, while pecifies not '
       'to.')
...
tf.app.flags.DEFINE_integer('augment_by_transposing', 1, 'If true during '
       'training shifts each data point by andom '
...
def check_proportion_in_mask(input_data, targets):
  masked_data, mask p.split(input_data, 2, 3)
  masked_note_count p.sum(targets ask)
  total_count p.prod(targets.shape)
  bout 0.0046, 0.018
  #print  masked_note_count loat(total_count), masked_note_count loat(np.sum(mask))
...
def run_epoch(supervisor,
  ...
  input_data, targets ata_tools.make_data_feature_maps(raw_data, config, encoder)

  ODO(annahuang): Leaves out last incomplete minibatch, needs wrap around.
  batch_size .batch_size
  num_batches nput_data.shape[0] // m.batch_size

  losses ummary_tools.AggregateMean('losses_%s' xperiment_type)
  losses_total ummary_tools.AggregateMean('losses_total_%s' %
            experiment_type)
  losses_mask ummary_tools.AggregateMean('losses_mask_%s' xperiment_type)
  losses_unmask ummary_tools.AggregateMean('losses_unmasked_%s' %
             (experiment_type))
  if not config.separate_instruments:
 accuracy_stats ummary_tools.AggregateInOutMaskPredictionPerformanceStats(
   '%s-prediction' xperiment_type, config.hparams.prediction_threshold)

  start_time ime.time()
  for step in range(num_batches):
 start_idx tep atch_size
 end_idx tart_idx atch_size
 x nput_data[start_idx:end_idx, :, :, :]
 y argets[start_idx:end_idx, :, :]

 # TODO(annahuang): Remove, added for testing.
 #check_mask ess.run([m.mask], {m.input_data: x, m.targets: y})[0]
 check_proportion_in_mask(x, y)

 # Evaluate the graph and run back propagation.
 results ess.run([m.predictions, m.loss, m.loss_total, m.loss_mask,
 ...
 losses_mask.add(loss_mask ask_size, mask_size)
 losses_unmask.add(loss_unmask nmask_size, unmask_size)

 if config.hparams.optimize_mask_only:
   losses.add(loss ask_size, mask_size)
 else:
   losses.add(loss, 1)
  if not config.separate_instruments:
 accuracy_stats.add(predictions, y, mask)

  ollect run statistics.
  if not config.separate_instruments:
 run_stats ccuracy_stats.get_aggregates_stats()
  else:
 run_stats ict()
  run_stats['loss_mask_%s' xperiment_type] osses_mask.mean
  run_stats['loss_unmask_%s' xperiment_type] osses_unmask.mean
  run_stats['loss_total_%s' xperiment_type] osses_total.mean
  run_stats['loss_%s' xperiment_type] osses.mean

  run_stats['perplexity_mask_%s' xperiment_type] p.exp(losses_mask.mean)
  run_stats['perplexity_unmask_%s' xperiment_type] 
   np.exp(losses_unmask.mean))
  run_stats['perplexity_total_%s' xperiment_type] p.exp(losses_total.mean)
  run_stats['perplexity_%s' xperiment_type] p.exp(losses.mean)

  ake summaries.
  if FLAGS.log_progress:
 summaries f.Summary()
 for stat_name, stat in run_stats.iteritems():
   value ummaries.value.add()
   value.tag tat_name
   value.simple_value tat
 supervisor.summary_computed(sess, summaries, epoch_count)

  heckpoint best model so far if running validation.
  if FLAGS.log_progress and experiment_type == 'valid' and (
   best_validation_loss un_stats['loss_%s' xperiment_type]):
 tf.logging.info('Previous best validation loss: %.3f.' %
     (best_validation_loss))
 best_validation_loss un_stats['loss_%s' xperiment_type]
 save_path s.path.join(
  FLAGS.run_dir, config.log_subdir_str,
  '%s-best_model.ckpt' onfig.hparams.name)
 ...
 print 'Storing best model so far with loss %.3f at %s.' 
  ...
  tf.logging.info('%s, epoch %d: perplexity, loss (mask): %.3f, %.3f, 
     ...
     un_stats['perplexity_mask_%s' xperiment_type],
     un_stats['loss_mask_%s' xperiment_type]))
  tf.logging.info('perplexity, loss (unmask): %.3f, %.3f, 
      (run_stats['perplexity_unmask_%s' xperiment_type],
     un_stats['loss_unmask_%s' xperiment_type]))
  tf.logging.info('perplexity, loss (total): %.3f, %.3f, 
      (run_stats['perplexity_total_%s' xperiment_type],
     un_stats['loss_total_%s' xperiment_type]))
  tf.logging.info('time taken: %.4f' time.time() tart_time))

  ODO(annahuang): Remove printouts.
  print '%s, epoch %d: perplexity, loss (mask): %.3f, %.3f,  (
   experiment_type, epoch_count,
     un_stats['perplexity_mask_%s' xperiment_type],
     un_stats['loss_mask_%s' xperiment_type]),
  print 'perplexity, loss (unmask): %.3f, %.3f,  (
     un_stats['perplexity_unmask_%s' xperiment_type],
     un_stats['loss_unmask_%s' xperiment_type]),
  print 'perplexity, loss (total): %.3f, %.3f,  (
     un_stats['perplexity_total_%s' xperiment_type],
     un_stats['loss_total_%s' xperiment_type]),
  print 'time taken: %.4f' time.time() tart_time)
...
def main(unused_argv):
 ...
 tf.logging.fatal('No input directory was provided.')
  ...
  oad hyperparameter settings, configs, and data.
  hparams yperparameters(
   model_name=FLAGS.model_name, num_layers=FLAGS.num_layers,
   num_filters=FLAGS.num_filters, batch_size=FLAGS.batch_size,
   use_residual=FLAGS.use_residual,
   augment_by_transposing=FLAGS.augment_by_transposing,
   augment_by_halfing_doubling_durations=FLAGS.augment_by_halfing_doubling_durations)

  config onfig_tools.PipelineConfig(hparams, FLAGS.maskout_method,
          LAGS.separate_instruments)
  ODO(annahuang): Use queues.
  train_data ist(data_tools.get_note_sequence_data(
   FLAGS.input_dir, 'train'))
  valid_data ist(data_tools.get_note_sequence_data(
   FLAGS.input_dir, 'valid'))
  print '# of train_data:', len(train_data)
  print '# of valid_data:', len(valid_data)

  pianoroll_encoder ianorolls_lib.PianorollEncoderDecoder(
   separate_instruments=FLAGS.separate_instruments,
   augment_by_transposing=FLAGS.augment_by_transposing)

  #if FLAGS.separate_instruments:
  assert train_data[0].shape[-1] == 4
  #else:
  assert train_data[0].shape[-1] == 1

  ODO(annahuang): Set this according to pitch range.
  best_validation_loss 0.0

  uild the graph and subsequently running it for train and validation.
  with tf.Graph().as_default() as graph:
 no_op f.no_op()

 # Builds input and target placeholders, initializer, and training graph.
 graph_objects uild_placeholders_initializers_graph(
  is_training=True, hparams=hparams)
 input_data, targets, initializer,  graph_objects

 # Build validation graph, reusing the model parameters from training graph.
 with tf.variable_scope('model', reuse=True, initializer=initializer):
   mvalid asicAutofillCNNGraph(
    is_training=False,
    hparams=hparams,
    input_data=input_data,
    targets=targets)

 # Instantiate upervisor and use it to start anaged session.
 saver   se default saver from supervisor.
 if not FLAGS.log_progress:
   saver one
 best_model_saver f.train.Saver()

 # Graph will be finalized after instantiating supervisor.
 sv f.train.Supervisor(
  graph=graph,
 ...
 with sv.managed_session("local") as sess:
   epoch_count 
   while epoch_count LAGS.num_epochs or not FLAGS.num_epochs:
  if sv.should_stop():
    break
  # Run training.
  run_epoch(sv, sess, m, train_data, pianoroll_encoder, config, m.train_op, 'train',
      epoch_count)

  # Run validation.
  if epoch_count onfig.eval_freq == 0:
    run_epoch(sv, sess, mvalid, valid_data, pianoroll_encoder, config, no_op, 'valid',
     epoch_count, best_validation_loss, best_model_saver)
  epoch_count += 1
