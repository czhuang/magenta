...
class PianorollsLibTest(tf.test.TestCase):
  ...
  def setUp(self):
 self.seqs ist(test_tools.get_small_bach_chorales_with_4_voices_dataset())

  def checkPianorollsFromMultipleConversionRounds(self, input_seq):
 """Check round trip starting from NoteSequence to pianoroll."""

 sorted_voices est_tools.collect_sorted_voices(input_seq, 'program')
 shortest_duration ianorolls_lib.find_shortest_duration([input_seq])
 ...
 pianoroll ianoroll_encoder_decoder.encode(input_seq)

 # Go from pianoroll back to note sequence.
 # pianoroll_to_program_map ianorolls_lib.reverse_mapping(
 # program_to_pianoroll_map)
 decoded_seq ianoroll_encoder_decoder.decode(pianoroll)

 # Go from decoded note sequence to pianoroll again.
 pianoroll_from_decoded_seq ianoroll_encoder_decoder.encode(decoded_seq)

 # Just comparing if all the notes are there.  Not yet checking if the
 # instruments are carrying the same notes.
 collapsed_pianoroll p.sum(pianoroll, axis=2)
 collapsed_pianoroll_from_decoded_seq p.sum(
  pianoroll_from_decoded_seq, axis=2)
 #print 'diffs:', np.sum(
 # np.abs(collapsed_pianoroll ollapsed_pianoroll_from_decoded_seq))
 ...
 second_decoded_seq ianoroll_encoder_decoder.decode(
 ...
 pianoroll_to_program ianorolls_lib.get_pianoroll_to_program_assignment(
  range(4))
