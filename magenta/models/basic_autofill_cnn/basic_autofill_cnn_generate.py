...
class SeedPianoroll(object):
 ...
 valid_data ata_tools.get_raw_data(path, 'valid')
 # Only use validation (unseen) data to seed the generation.
 self._raw_data ist(valid_data)
 self._num_pieces en(self._raw_data)
 self._config onfig
 ...
 random_piece p.random.choice(self._raw_data)
...
def retrieve_model(wrapped_model=None,
     askout_method_str='random_pitch_range'):
 ...
 checkpoint_fpath s.path.join(tf.resource_loader.get_data_files_path(),
         'checkpoints',
         '%s_best_model.ckpt' onfig.hparams.name)
