...
class BasicAutofillCNNGraphTest(tf.test.TestCase):
  ...
  def setUp(self):
    ...
    self.config = config_tools.get_checkpoint_config(model_name='SmallTest')
  ...
  def testTrainingModel(self):
    """Training a random step and check that all losses are within ranges."""
    wrapped_model = self.wrapped_model
    model = wrapped_model.model
    # Take a training step.
    with wrapped_model.graph.as_default():
      _ = wrapped_model.sess.run([model.train_op],
